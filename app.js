...
async function api(path, options = {}) {
  if (!API_BASE) throw new Error('Backend ešte nie je nastavený. Frontend je pripravený na Cloudflare API.');
  ...
  if (!res.ok) throw new Error(data.error || data.message || 'API chyba');
  return data;
}
...
function bindAuth(){
  ...
  loginForm?.addEventListener('submit', async (e) => {
    ...
    try {
      if (API_BASE) await api('/api/auth/login', { method:'POST', body: JSON.stringify({ email, password }) });
  ...
  registerForm?.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (API_BASE) await api('/api/auth/register', { method:'POST', body: JSON.stringify({ email, password }) });
      else mockRegister(email, password);
      setStatus(qs('registerStatus'), 'Účet bol vytvorený. Po úhrade ho aktivuješ v admine.', 'ok');
    } catch (err) {
  ...
  qs('logoutBtn')?.addEventListener('click', async () => {
    try {
      if (API_BASE) await api('/api/auth/logout', { method:'POST' });
      else mockLogout();
    } catch (e) {}
    location.href = 'index.html';
  });
...
function populateDashboard(){
  ...
  if (companySelect) {
    ...
    mockState.companies.forEach(c => {
      ...
      o.value = c.id; o.textContent = `${c.companyName} • ${c.iban}`;
  ...
  qs('quickQrForm')?.addEventListener('submit', (e) => {
    ...
    if (mockState.me.license.status !== 'active') {
      return setStatus(qs('quickQrStatus'), 'Účet ešte nie je aktivovaný. Po zaplatení ho aktivuješ v admine.', 'err');
    }
    ...
    setStatus(qs('quickQrStatus'), 'Frontend je pripravený. Reálne QR bude fungovať po napojení na Cloudflare backend.', 'ok');
...
function bindCompanies(){
  ...
  form.addEventListener('submit', (e) => {
    ...
    const ix = mockState.companies.findIndex(x => x.id === payload.id);
    if (ix >= 0) mockState.companies[ix] = payload; else mockState.companies.push(payload);
    saveMock();
    renderCompanies();
    resetCompanyForm();
...
function bindGenerator(){
  ...
  form.addEventListener('submit', (e) => {
    ...
    localStorage.setItem('mock_qr_count', String(Number(localStorage.getItem('mock_qr_count') || '0') + 1));
    setStatus(qs('generatorStatus'), 'Toto je len vizuálny náhľad. Reálny PAY by square QR doplníme po napojení na Cloudflare.', 'ok');
  });
}
...
function bindAdmin(){
  ...
  if (!mockState.adminUsers.length && mockState.me.email) {
    mockState.adminUsers.push({
      id: uid(),
      email: mockState.me.email,
      role: mockState.me.role,
      status: mockState.me.status,
      licenseStatus: mockState.me.license.status,
      createdAt: new Date().toISOString()
    });
    saveMock();
  }
  ...
  const render = () => {
    ...
    list.querySelectorAll('[data-activate]').forEach(btn => btn.addEventListener('click', () => {
      const user = mockState.adminUsers.find(x => x.id === btn.dataset.activate);
      if (!user) return;
      user.status = 'active';
      user.licenseStatus = 'active';
      if (mockState.me.email === user.email) {
        mockState.me.status = 'active';
        mockState.me.license.status = 'active';
        mockState.me.license.activatedAt = new Date().toISOString().slice(0,10);
      }
      saveMock();
      render();
    ...
    list.querySelectorAll('[data-block]').forEach(btn => btn.addEventListener('click', () => {
      const user = mockState.adminUsers.find(x => x.id === btn.dataset.block);
      if (!user) return;
      user.status = 'blocked';
      user.licenseStatus = 'blocked';
      if (mockState.me.email === user.email) {
        mockState.me.status = 'blocked';
